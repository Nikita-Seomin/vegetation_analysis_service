...
if __name__ == "__main__":
    ...
    #     print(
    #         "example python3  {} --red_band red.tif --nir_band nir.tif --path . --output ndvi.tif".format(sys.argv[0]))
    #     sys.exit(-1)
    # else:
    #     usage = "usage: %prog [options] "
    #     parser = OptionParser(usage=usage)
    #     parser.add_option("-r", "--red_band", dest="red", action="store", type="string",
    #                       help="Name of the red band image.", default=None)
    #     parser.add_option("-n", "--nir_band", dest="nir", action="store", type="string",
    #                       help="Name of the nir band image.", default=None)
    #     parser.add_option("-p", "--path", dest="path", action="store", type="string", help="Path to data.",
    #                       default=None)
    #     parser.add_option("-o", "--output", dest="output", action="store", type="string",
    #                       help="Name of the output image.", default=None)
    #
    #     (options, args) = parser.parse_args()
        ...
        z = zipfile.ZipFile('./dataFromSpace.zip')  # Flexibility with regard to zipfile
        for c in z.namelist():
            # print(c)
            if c == 'B04_8.tiff':
                red = rasterio.open('zip+file:./dataFromSpace.zip/' + c, "r")
            else:
                nir = rasterio.open('zip+file:./dataFromSpace.zip/' + c, "r")
        ...
        with rasterio.open(os.path.join('.', name), "w", **metadata) as dst:
